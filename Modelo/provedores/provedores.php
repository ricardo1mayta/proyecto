...
public function get_provedores($txt,$sed) 
    {
         $sql = "SELECT PV_C_CODIGO,PV_D_NOMBRES,PV_C_DOC,PV_D_DIRECCION FROM sm_provedores WHERE SED_C_CODIGO='$sed' AND PV_E_ESTADO>0 AND PV_D_NOMBRES LIKE '".$txt."%' LIMIT 10;";
         $rows=$this->db->query($sql);  
        return $rows;
    ...
    {
         $sql = "SELECT * FROM  `sm_provedores`  WHERE SED_C_CODIGO='$sed' AND PV_E_ESTADO>0";
         $rows=$this->db->query($sql);  
        return $rows;
