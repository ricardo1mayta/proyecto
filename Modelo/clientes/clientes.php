...
public function delete_cliente($us,$id) {
    ...
    {
        $sql = "SELECT `CLI_C_CODIGO`, `CLI_D_NOMBRE`,`CLI_D_APELLIDOS`, `CLI_D_DOC`, `CLI_D_DIRECCION`, `CLI_T_TELEFONO`, `CLI_D_CORREO` FROM `sm_cliente` WHERE `SED_C_CODIGO`='$sed' and  `CLI_E_ESTADO` > 0;";
       $rows=$this->db->query($sql);  
        return $rows;
    }
...
public function get_clientes($txt,$sed) 
    {
         $sql = "SELECT `CLI_C_CODIGO`,concat(`CLI_D_NOMBRE`,' ',`CLI_D_APELLIDOS`) as NOMBRE ,`CLI_D_DIRECCION`,`CLI_D_DOC` FROM `sm_cliente` WHERE `SED_C_CODIGO`='$sed' and concat(`CLI_D_NOMBRE`,' ',`CLI_D_APELLIDOS`) LIKE '".$txt."%' LIMIT 10;";
         $rows=$this->db->query($sql);  
        return $rows;
